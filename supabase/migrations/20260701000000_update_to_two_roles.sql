...
-- Step 3: Recreate the app_role enum type with the new roles (petani, pembeli)
DROP TYPE IF EXISTS public.app_role;
CREATE TYPE public.app_role AS ENUM ('petani', 'pembeli');

-- Step 4: Migrate any legacy role names to the new role names in the user_roles table
UPDATE public.user_roles SET role = 'pembeli' WHERE role = 'calon_petani';
...
BEGIN
  ...
  v_role := COALESCE(
    CASE 
      ...
      WHEN NEW.raw_user_meta_data->>'role' = 'pemula' THEN 'pembeli'::public.app_role
      WHEN NEW.raw_user_meta_data->>'role' = 'pembeli' THEN 'pembeli'::public.app_role
      WHEN NEW.raw_user_meta_data->>'role' = 'calon_petani' THEN 'pembeli'::public.app_role
      WHEN NEW.raw_user_meta_data->>'role' = 'petani' THEN 'petani'::public.app_role
      WHEN NEW.raw_user_meta_data->>'role' = 'seller' THEN 'petani'::public.app_role
      WHEN NEW.raw_user_meta_data->>'role' = 'penjual' THEN 'petani'::public.app_role
